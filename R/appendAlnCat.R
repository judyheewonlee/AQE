#' appendAlnCat.R
#'
#' \code{<function>} Append the alignment ID with the corresponding reference ID it belongs to
...
#'
#' @return The database with the alignment category appended to the alnCategory data frame
...
appendAlnCat <- function(database, alnID, directory) {
  ...
  database$alnCategory <- rbind(database$alnCategory,
                                data.frame(alnID = alnID, refID = directory,

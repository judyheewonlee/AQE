#' appendSeqCat.R
#'
#' \code{<function>} Append the sequence ID and it's corresponding alignment ID to the provided
#' database.
#'
#' Details.
#'
...
#'
#' @return The database with the sequence ID and it's corresponding alignment ID appended to
#' the data frame seqCategory contained in the the database.

appendSeqCat <- function(database, readaln, alnID) {

  # Add the alignment ID for each given sequence
  for (seqID in readaln$nam) {
    database$seqCategory <- rbind(database$seqCategory, data.frame(seqID = seqID,
                                                                   alnID = alnID,
